...
| `directRun?: { enabled?: boolean }` | Controls Direct Run for this viewer. It defaults to `true`. | Set `enabled: false` to keep serving the graph without registering Direct Run or trace-history routes. The viewer graph also omits Direct Run metadata, so the UI does not advertise runnable methods. |

### Direct Run

With Direct Run enabled, the viewer can invoke only public provider methods
advertised in that provider's Direct Run metadata. Constructors, Nest lifecycle
hooks, inherited methods, non-allowlisted methods, methods shadowed on an
instance, and TypeScript-`private` methods are refused.

The request body is limited to 1 MiB of encoded bytes. Oversized requests
receive a generic `413` response; UTF-8 decoding works transparently across
request chunks. Completed runtime-trace history retains at most 100 traces in
