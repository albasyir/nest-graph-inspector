...
Tokens gate every endpoint the inspector installs, and nothing else in your
application. For a Viewer output, the graph routes below are installed; the
Direct Run and history routes are installed only when that output's
`directRun.enabled` is not `false` (it defaults to `true`):

| Endpoint | What it serves |
| --- | --- |
| `GET /__graph-inspector/information.json` | Which inspector version is installed and what it serves |
| `GET /__graph-inspector/output.json` | Your dependency graph |
| `GET /__graph-inspector/output.schema.json` | The JSON Schema that graph conforms to |
| `GET /__graph-inspector/output.md` | The same graph as Markdown |
| `POST /direct-run` | Invokes an advertised provider method; exists only when Direct Run is enabled |
| `GET /direct-run/histories` | Completed Direct Run traces in full; exists only when Direct Run is enabled |
| `GET /direct-run/history/index.json` | A summary list of completed traces; exists only when Direct Run is enabled |
| `GET /direct-run/history/<traceId>` | One completed trace; exists only when Direct Run is enabled |

The graph paths above are relative to the Viewer output's `path`.
`/direct-run` is the default Direct Run path and follows `directRun.path` if
you change it. With `directRun.enabled: false`, the graph remains available but
none of the Direct Run or history paths exist, and its graph omits Direct Run
metadata. The AI chat is not on this list because it is not an endpoint: the
model runs inside your browser and reads the graph through `output.md`, which
is already gated.

### How a token travels

A token is issued when your application starts and is embedded directly in the
viewer link printed to your console, so opening that link just works. That link
is the only thing that hands a token over: the viewer reads it once, keeps it
for the tab, and immediately rewrites its own address bar without it. From then
on it authenticates with the `x-graph-inspector-token` header on every
follow-up request, Direct Run included.

That means the URL you can copy out of the viewer, share, or leave in your
history is not a working credential — only the link printed in your console is.
...
you would a password and prefer the header form for anything scripted — which
is what the viewer itself does.
