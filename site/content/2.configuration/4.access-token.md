...
Every endpoint the inspector installs, and nothing else in your application:
...
| `POST /direct-run` | Invokes a provider method on your running application |
| `GET /direct-run/histories` | Every recorded trace of a previous Direct Run, in full |
| `GET /direct-run/history/index.json` | A summary list of those traces |
| `GET /direct-run/history/<traceId>` | One recorded trace |

The paths above are the defaults; they follow the viewer output's `path` and
`directRun.path` if you change them. The AI chat is not on this list because it
is not an endpoint: the model runs inside your browser and reads the graph
through `output.md`, which is already gated.
