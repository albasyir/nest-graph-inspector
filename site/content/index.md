...
#description
Ask graph and trace questions in plain language. In the real viewer, answers are grounded in your loaded runtime graph and generated by a model running inside your browser, so all you need is a WebGPU-capable browser with enough GPU memory for the model you pick.
...
#features
  ...
  #description
  Nothing to install and no server to run. The model downloads once into your browser cache, then runs on your own GPU, so the graph never leaves your machine.
  :::
