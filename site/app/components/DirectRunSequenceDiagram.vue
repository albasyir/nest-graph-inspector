...
function getSpanLabel(span: RuntimeTrace["spans"][number]): string {
  const owner = span.className || span.resource || span.moduleName || "Unknown";
...
const timelineSeries = computed(() => {
  ...
  const data = [...props.trace.spans]
    ...
    .map<TimelinePoint>((span) => {
      const start = Number.isFinite(toMs(span.startedAt))
        ? toMs(span.startedAt)
        : fallbackStart;
...
const chartHeight = computed(() =>
  Math.max(260, props.trace.spans.length * 48 + 96),
);
...
const chartOptions = computed<ApexOptions>(() => ({
  ...
  xaxis: {
    ...
    labels: {
      ...
      datetimeUTC: false,
      formatter: (value) =>
        `${Math.max(0, Number(value) - toMs(props.trace.startedAt))} ms`,
    },
  ...
  yaxis: {
    labels: {
      maxWidth: 190,
      style: {
        colors: "var(--ui-text-highlighted)",
        fontSize: "12px",
        fontWeight: 600,
      },
    },
  ...
  dataLabels: {
    ...
    formatter: (_value, options) => {
      const point = options
        ? timelineSeries.value[0]?.data[options.dataPointIndex]
        : null;
  ...
  tooltip: {
    ...
    custom: ({ dataPointIndex }) => {
      ...
      const subtitle = span.errorMessage || span.moduleName || "Unknown module";
